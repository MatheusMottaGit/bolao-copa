<x-layouts::app :title="$group->name">
    <div class="flex flex-col gap-6 max-w-4xl mx-auto">
        ...
        <div class="flex items-start justify-between gap-4">
            <div>
                <a href="{{ route('groups.index') }}" class="mb-2 inline-flex items-center gap-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 transition hover:text-[#C9920A]">
                ...
                <div class="mt-2 flex items-center gap-2">
                    ...
                    <span class="rounded bg-[#C9920A]/10 px-2 py-0.5 font-mono text-xs font-bold text-[#C9920A]">{{ $group->code }}</span>
                </div>
            </div>
            <a href="{{ route('ranking.show', $group) }}"
                class="inline-flex shrink-0 items-center gap-2 rounded-lg border border-[#C9920A] px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#C9920A] transition hover:bg-[#C9920A]/10">
        ...
        @forelse ($games as $game)
            ...
            <div class="rounded-xl border border-zinc-800 bg-slate-900 overflow-hidden">
                ...
                <div class="flex items-center justify-between border-b border-zinc-800 px-5 py-2.5">
                    ...
                    @else
                        <span class="inline-flex items-center gap-1.5 text-xs font-semibold text-[#C9920A]">
                ...
                <div class="grid grid-cols-3 items-center gap-4 px-6 py-5">
                    <div class="flex flex-col items-center gap-2">
                        ...
                        @endif
                        <p class="text-center text-sm font-bold text-white">{{ $game->home_team }}</p>
                    </div>
                    ...
                    <div class="flex flex-col items-center gap-2">
                        ...
                        @else
                            <div class="h-10 w-10 rounded-lg bg-zinc-800"></div>
                        @endif
                        <p class="text-center text-sm font-bold text-white">{{ $game->away_team }}</p>
                    </div>
                </div>
        ...
        @empty
            <flux:callout icon="information-circle">Nenhum jogo cadastrado ainda.</flux:callout>
        @endforelse
