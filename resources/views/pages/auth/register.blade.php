<x-layouts::auth :title="__('Cadastro')">
    <div class="w-full max-w-lg overflow-visible">
        ...
        <div class="relative overflow-visible px-8 py-10 sm:px-12">
            ...
            <form method="POST" action="{{ route('register.store') }}" class="space-y-6">
                ...
                <div>
                    <label for="username"
                        class="mb-2 flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.18em] text-slate-600">
                    ...
                    <input id="username" name="username" value="{{ old('username') }}" type="text" required autofocus
                ...
                <div>
                    ...
                    <div x-data="{ show: false }" class="relative">
                        <input id="password" name="password" :type="show ? 'text' : 'password'" required autocomplete="new-password"
                            ...
                            class="w-full rounded-lg border border-zinc-700 bg-slate-950 px-5 py-4 pr-12 text-sm text-white outline-none transition focus:border-[#C9920A] focus:ring-2 focus:ring-[#C9920A]/15" />
                        <button type="button" @click="show = !show" :aria-label="show ? 'Ocultar senha' : 'Mostrar senha'" tabindex="-1"
                            class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 transition hover:text-[#C9920A]">
            ...
            <p class="mt-6 text-center text-sm text-slate-500">
                Já tem conta?
                <a href="{{ route('login') }}" class="font-semibold text-[#C9920A] hover:text-[#b47f0a]">Entrar</a>
            </p>
        </div>
    </div>
</x-layouts::auth>